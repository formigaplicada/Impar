...
async function gerarMandatoPDF(data) {
  ...
  // Template PDF oficial SEPA em base64
  ...
  const NAVY  = rgb(0.004, 0.086, 0.251)  // #011640
  const BLACK = rgb(0, 0, 0)
  const WHITE = rgb(1, 1, 1)
  const LIME  = rgb(0.784, 0.855, 0)      // #C8DA00

  const H = 842  // altura da página

  // ── Helper: desenhar texto numa posição (coordenadas pdfplumber → pdflib) ──
  const txt = (text, x, topY, size = 9, font = fontReg, color = BLACK) => {
    if (!text) return
  ...
  // ── Bloco Nome & Logo do Credor (canto superior direito) ──────────────────
  // Área: x ~390-560, top ~38-75
  page.drawRectangle({ x: 388, y: H - 80, width: 175, height: 55, color: NAVY })
  txt('Rede Impar, Lda', 395, 45, 10, fontBold, LIME)
  txt('PT18ZZZ114843', 395, 58, 8, fontReg, WHITE)
  txt('Gestao de Condominios', 395, 68, 7, fontReg, WHITE)

  // ── Referência ADD ────────────────────────────────────────────────────────
  // Linha de referência: top ~63, x ~42
  txt(data.adc, 42, 63, 8, fontBold, NAVY)

  // ── Identificação do Devedor ──────────────────────────────────────────────
  // Nome devedor: top ~193, x ~176
  txt(data.nomeDevedor, 176, 193, 9, fontReg, BLACK)

  // Morada: top ~215, x ~176
  txt(data.moradaDevedor, 176, 215, 9, fontReg, BLACK)

  // Código Postal: top ~238, x ~176
  txt(data.cpDevedor, 176, 238, 9, fontReg, BLACK)

  // Cidade: top ~238, x ~320
  txt(data.cidadeDevedor, 320, 238, 9, fontReg, BLACK)

  // País: top ~260, x ~176
  txt('Portugal', 176, 260, 9, fontReg, BLACK)

  // IBAN: top ~284, x ~176
  txt(data.iban ? data.iban.replace(/(.{4})/g, '$1 ').trim() : '', 176, 284, 9, fontBold, BLACK)

  // BIC: top ~306, x ~176
  txt(data.bic || '', 176, 306, 9, fontReg, BLACK)

  // ── Identificação do Credor ───────────────────────────────────────────────
  // Nome credor: top ~335, x ~176
  txt('Rede Impar, Lda', 176, 335, 9, fontReg, BLACK)

  // Creditor ID: top ~357, x ~176
  txt('PT18ZZZ114843', 176, 357, 9, fontReg, BLACK)

  // Morada credor: top ~380, x ~176
  txt('Av. Dr. Manuel de Arriaga, N 5 F', 176, 380, 9, fontReg, BLACK)

  // CP credor: top ~403, x ~176
  txt('2900-474', 176, 403, 9, fontReg, BLACK)

  // Cidade credor: top ~403, x ~320
  txt('Setubal', 320, 403, 9, fontReg, BLACK)

  // País credor: top ~425, x ~176
  txt('Portugal', 176, 425, 9, fontReg, BLACK)

  // ── Tipo de pagamento ─────────────────────────────────────────────────────
  // Checkbox recorrente: top ~444, x ~358
  txt('[X]', 358, 444, 9, fontBold, NAVY)

  // ── Local e data de assinatura ────────────────────────────────────────────
  // Localidade: top ~471, x ~176
  txt(data.cidadeDevedor || 'Portugal', 176, 471, 9, fontReg, BLACK)

  // Data DD MM AA: top ~471
  const d = data.signedAt
  const dd = String(d.getDate()).padStart(2, '0')
  const mm = String(d.getMonth() + 1).padStart(2, '0')
  const aa = String(d.getFullYear()).slice(2)
  txt(dd, 407, 471, 9, fontReg, BLACK)
  txt(mm, 422, 471, 9, fontReg, BLACK)
  txt(aa, 437, 471, 9, fontReg, BLACK)

  // ── Assinatura ────────────────────────────────────────────────────────────
  // Área de assinatura: top ~486-500, x ~176-560
  if (data.signaturePng) {
    try {
      ...
      page.drawImage(img, {
        x: 176, y: H - 500, width: 200, height: 40
      })
    } catch (_) {}
  }

  // ── Rodapé de integridade ─────────────────────────────────────────────────
  const timestampStr = `Assinado digitalmente em ${data.signedAt.toISOString()} | IP: ${data.signedIp}`
  page.drawText(timestampStr, {
    x: 42, y: 8, size: 6, font: fontReg, color: rgb(0.5, 0.5, 0.5)
  })
